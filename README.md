...
一座等角控制平面城市，帶一筆 ARM 要求走過 AZ-104 部署路徑。推車離開 Entra 大門，載著一位使用者、一枚權杖與一個期望資源：在角色櫃櫈綁定、在原則大廳裁決、打上標籤與鎖定、放到子網路、依存取路徑分岔、在倉庫複製、在運算棚佈建、從瞭望塔監看，並保留在保險庫。
...
| 角色櫃櫈 | 身分 | 由角色 + 範圍聯集動作；管理群組→訂用帳戶→資源群組繼承 |
